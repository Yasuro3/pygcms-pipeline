...
- Packaging correction: restored the production localhost bridge from the original optimized distribution (`scripts/nist_mssearch_bridge_server.py`, `scripts/nist_mssearch_bridge.py`, the optional DLL backend, and Windows launch/diagnostic scripts). The bridge now serves `software/index.html`, accepts the application's `max_candidates` field, and is covered by `tests/test_nist_bridge_contract.py`. No NIST executable, library, spectrum, or licence material is included.
